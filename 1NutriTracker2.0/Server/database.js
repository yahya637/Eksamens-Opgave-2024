...
async createUserActivity(userActivityData) {
    ...
    try {
        ...
        request.input('DurationMinutes', sql.Int, userActivityData.DurationMinutes);
...
// Modified SQL query to include ORDER BY clause
const result = await request.query(`
...
TotalKcalBurned: item.total_kcal_burned,
TimeAdded: item.timeAdded,
DateAdded: item.dateAdded
})),
...
DateAdded: bmrResult.recordset[0].dateAdded
}] : []
};
} catch (error) {
